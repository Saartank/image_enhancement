...
def build_net():
    ...
    new_pool0s = tf.keras.layers.concatenate([pool0s, inp_uv1, inp_y2], axis=3)
    pool1s, conv1s, conv1 = encode_block_light(new_pool0s, dims)
	
    print(f'Enc 1 --> pool1s.shape={pool1s.shape}, conv0.shape={conv1.shape}')
    new_pool1s = tf.keras.layers.concatenate([pool1s, inp_uv2, inp_y3], axis=3)
    ...
    pool3s, conv3s, conv3 = encode_block_light(new_pool2s, dims*4)
...
class MyModel(keras.Model):
    ...
    def compute_losses(self, output_y,output_uv, y_temp_gt_patch,uv_temp_gt_patch):
        ...
        for i in range(levels+1):
            y_loss, uv_loss = l1_loss(pred_y_pym[i], gt_y_pym[i]), l1_loss(pred_uv_pym[i], gt_uv_pym[i])
