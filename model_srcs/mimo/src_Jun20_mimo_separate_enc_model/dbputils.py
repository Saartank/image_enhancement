...
def augment_samples_yuv(input_images, gt_image):
    t1, t2, t3 = False, False, False
    if np.random.randint(2, size=1)[0] == 1:  # random flip
        gt_image = np.flip(gt_image, axis=1)
        #gt_image_raw = np.flip(gt_image_raw, axis=1)
...
def augment_samples_dump(input_images, gt_image):
    t1, t2, t3 = False, False, False
    if np.random.randint(2, size=1)[0] == 1:  # random flip
        gt_image = np.flip(gt_image, axis=1)
        t1 = True
...
def crop_samples_yuv(input_images, gt_image, ps=512):
    in_image = input_images[0]
    H = in_image.shape[1]
    W = in_image.shape[2]
    xx = np.random.randint(0, W - ps)
    ...
    #gt_raw_patch = gt_image_raw[:, yy:yy + ps, xx:xx + ps, :]
    return np.array(input_patches), gt_patch

def crop_samples_dump(input_images, gt_image, ps=512, raw_ratio=2):
    in_image = input_images[0]
    H = in_image.shape[1]
    W = in_image.shape[2]
    xx = np.random.randint(0, W - ps)
...
def pack_raw_dump(raw, black_level=0, maxpix=1023):
    # pack Bayer image to 4 channels
    # im = raw
    # im = raw.raw_image_visible.astype(np.float32)
    im =raw
    im = np.maximum(im - black_level, 0) / (maxpix - black_level)
...
def pack_raw_rogue(raw, black_level=63, maxpix=128):
    # pack Bayer image to 4 channels
    im = raw.raw_image_visible.astype(np.float32)
    print(im)
    print(np.max(im))
    ...
    g2 = im[1:H:2, 0:W:2, :]
    #rgbg
    out = np.concatenate((r, g1, b, g2), axis=2)
...
def get_burst_paths(in_path, nb):
    paths = []
    complete = True
    for i in range(nb):
...
def get_burst_paths_yuv(in_path, nb):
    paths = []
    complete = True
    for i in range(nb):
        path = in_path.replace("_00_","_0%d_"%i)
